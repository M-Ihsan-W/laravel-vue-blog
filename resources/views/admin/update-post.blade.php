...
@section('main')
  <h1 class="my-3">Update Posts</h1>
  <form action="{{ route('update.posts') }}" method="POST" enctype="multipart/form-data">
    @csrf
    <div class="mb-3">
      <label for="writer" class="form-label">Writer</label>
      <input type="hidden" name="id" value="{{ $post->id }}">
      <input type="text" name="writer" class="form-control @error('writer') is-invalid @enderror" id="writer" value="{{ $post->writer }}">
      @error('writer')
    ...
    <div class="mb-3">
      ...
      <input type="text" name="category" class="form-control @error('category') is-invalid @enderror" id="category" value="{{ $post->category }}">
      @error('category')
        <div class="invalid-feedback">
          {{ $message }}
        </div>
